...
async function bootstrap() {
  ...
  const rmqOptions: RmqOptions = app.get(ServiceClientRmqConfig).createClientOptions();
  app.connectMicroservice(rmqOptions, { inheritAppConfig: false });

  const tcpOptions: TcpOptions = app.get(SERVICE_TCP_CONFIG_KEY(0)).createClientOptions();
